...
This URL can now be used in your `cave_api` as the source for your raster tiles. A working example using custom tiles can be found in `your_app/cave_api/examples/map_custom_tiles.py`, which demonstrates four different map projections in a 2x2 grid format. This includes three grid maps with different aspect ratios (1:1, 2:1, 1:2) and a simple warehouse map.
