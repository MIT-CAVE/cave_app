            ...
            if pre_auth:
                if settings.LOG_AUTH:
                    settings.AUTH_LOGGER.warning(
                        f"Auth - {user_obj.username}: Attempted login on locked account"
                    )
            if datetime.now(timezone.utc) < user_obj.locked_out_until:
                lockout_minutes = int(
                    (user_obj.locked_out_until - datetime.now(timezone.utc)).total_seconds() // 60
                    + 1
                )
                if lockout_minutes > 1440:
                    raise ValidationError(
                        f"Due to too many unsuccessful login attempts this account has been fully locked out. Please contact an admin for assistence."
                    )
                raise ValidationError(
                    f"Due to too many unsuccessful login attempts this account has been locked for the next {lockout_minutes} minute{'s' if lockout_minutes > 1 else ''}."
                )
