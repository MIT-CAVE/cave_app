...
class CustomSecurityMiddleware:
    ...
    def __call__(self, request):
        ...
        response['Content-Security-Policy'] = f"default-src 'self' {" ".join(settings.CONTENT_SOURCE_URLS)} 'unsafe-inline'; img-src 'self' data: {" ".join(settings.CONTENT_SOURCE_URLS)}; frame-src 'self' {settings.STATIC_APP_URL}; form-action 'self'; frame-ancestors 'none';" 
