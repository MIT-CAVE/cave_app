...
if [ "$(uv run python ./manage.py check --deployment_type development | grep "System check identified no issues" | wc -l)" -eq "0" ]; then
...
uv run python "$APP_DIR/manage.py" migrate --deployment_type development 2>&1 | pipe_log "DEBUG"
uv run python "$APP_DIR/manage.py" createcachetable 2>&1 | pipe_log "DEBUG"
...
# Nuke the Cache
uv run python "$APP_DIR/manage.py" clearcache --deployment_type development 2>&1 | pipe_log "DEBUG"

# Clear the current DB
uv run python "$APP_DIR/manage.py" flush --noinput --deployment_type development 2>&1 | pipe_log "DEBUG"
# Generate data
uv run python "$APP_DIR/utils/data_gen.py" --deployment_type development 2>&1 | pipe_log "DEBUG"
