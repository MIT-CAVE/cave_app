...
<a name="deprecat-unit">`unit`</a> (_Deprecated in `0.2.0`_) | | A unit of measurement that is displayed next to the numeric value in an item ([`'num'` prop](props.md#num), [KPI](../all_keys/kpis.md), or [stat](../all_keys/stats.md)). The use of this property is deprecated as of version `0.2.0` in favor of [`numberFormat.unit`](number_format.md#unit). <!-- REVIEW: Update this note when dropping support for `unit` in 1.0.0 -->
