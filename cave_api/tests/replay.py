...
def parse_log(log_path):
    ...
    if not isinstance(raw_events, list):
        raise ValueError(f"Expected log file to contain a list of events, but got: {type(raw_events)}")
    events = []
    session_ids = set()
    for i, raw_event in enumerate(raw_events):
        if not isinstance(raw_event, dict):
            raise ValueError(f"Expected each event to be a dictionary, but got: {type(raw_event)} at index {i}")
        event = {}
        ...
        events.append(event)
    assert len(session_ids) == 1, "Log contains events from multiple sessions, which is not supported."
...
def replay_events(events, validate_each_step=False):
    ...
    validate(session_data, message="Initial session data validation failed after init command.", do_validate=validate_each_step)
    for event in events:
        ...
        validate(session_data, message=f"Session data validation failed after processing event with API command: {event.get('api_command')}", do_validate=validate_each_step)
    validate(session_data, message="Final session data validation failed after replay.", do_validate=True)
    return session_data
